...
function loadMore(res) {
    res.forEach(function (bot) {
        ...
        <div class="card">
            ...
            <a href="/bots/${bot.botid}" class="button small">Ver info del bot</a>
        </div>`
...
function search() {
    ...
    if (cards) {
        ...
        if (cardsVisible === 0) {
            document.getElementById('searchMore').innerHTML = `No se encontraron bots. Te gustaria <a href="/bots/search/?q=${s}">buscar todos los bots</a>?`;
            document.getElementById('searchMore').style.display = "block";
        } else {
            document.getElementById('searchMore').innerHTML = `Te gustaria <a href="/bots/search/?q=${s}">buscar todos los bots</a>`
            document.getElementById('searchMore').style.display = "block";
        }
