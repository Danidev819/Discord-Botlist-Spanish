...
route.post("/:id", auth, async (req, res) => {
    ...
    if (original && original.state !== "deleted")
        return res.json({ success: false, message: "Tu bot ya existe en la lista.", button: { text: "Editar", url: `/bots/edit/${bot.id}` } });
    ...
    try {
        ...
        return res.json({ success: true, message: "Tu bot ha sido agregado" })
    } catch (e) {
        return res.json({ success: true, message: "Tu bot ha sido agregado" })
    }
});
