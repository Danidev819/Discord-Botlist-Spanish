...
route.patch("/:id", auth, async (req, res) => {
    ...
    req.app.get('client').channels.cache.get(server.mod_log_id).send(`<@${req.user.id}> ha actualizado a <@${bot.botid}>`)
    return res.json({success: true, message: "Bot agregado", url: `/bots/${bot.botid}`})
});
